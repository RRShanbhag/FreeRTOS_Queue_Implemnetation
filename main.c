...
#define mainCREATE_TWO_TASKS_ASSIGNMENT_1 1
...
static void matrix_task(void *p);
static void reader_task(void *p);

/* Global Variables*/
traceLabel xTickTraceUserEvent;
TaskHandle_t xMatrixHandle;
TaskHandle_t xReaderHandle;
BaseType_t xMatrix;
BaseType_t xReader;
...
int main( void )
{
	...
	Queue_Handle = xQueueCreate(10, sizeof(double));

	xMatrix = xTaskCreate(matrix_task,	/* The function that implements the task. */
		"Matrix Task", 					/* The text name assigned to the task - for debug only as it is not used by the kernel. */
		1000, 							/* The size of the stack to allocate to the task. */
		NULL, 							/* The parameter passed to the task - not used in this simple case. */
		3,								/* The priority assigned to the task. */
		&xMatrixHandle);				/* The task handle is not required, so NULL is passed. */
										
	xReader = xTaskCreate(reader_task, "Reader Task", 1000, NULL, 1, &xReaderHandle);
...
static void matrix_task(void *p)
{
	...
	while (1) {
		...
		*/
		...
		printf("Sending Data to reader\n");
		for (i = 0; i < SIZE; i++) {
			for (j = 0; j < SIZE; j++) {
				//printf("%d  ",(&(*c+i)[j]));
				if (!xQueueSend(Queue_Handle,(&c[i][j]), 50))
				{
	...
	while (1)
	{
		...
		for (col = 0; col < SIZE; col++) {
			...
			for (row = 0; row < SIZE; row++) {
				if (xQueueReceive(Queue_Handle, &rx_data, 100)) 
				{
					printf("%lf  ", rx_data);
				}
				else 
...
void vApplicationIdleHook( void )
{
	...
	*/
	...
	#if ( mainCREATE_TWO_TASKS_ASSIGNMENT_1 != 1 )
	{
...
void vApplicationTickHook( void )
{
	...
	//CommRunTime = xTaskDetails.ulRunTimeCounter;
	//printf("vAppTHook: EndCommCounter = %d\n", EndCommCounter);
	...
	trace recording. */
	//printf("vAppTHook: vTraceEvent Start \n");
